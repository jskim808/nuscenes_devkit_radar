...
class DetectionEval:
    ...
    def __init__(self,
        ...
        self.cfg = config
        gt_dir,_ = osp.split(result_path)
        gt_path = osp.join(gt_dir,'gt_nusc.json')
        with open(gt_path) as f:
            gt_boxes = json.load(f)

        all_gt = EvalBoxes()
        for sample_token, gt_boxes in gt_boxes.items():
            sample_boxes=[]
            for gt_box in gt_boxes:
                sample_boxes.append(
                        DetectionBox(
                            sample_token=gt_box['sample_token'],
                            translation = gt_box['translation'],
                            size = gt_box['size'],
                            rotation = gt_box['rotation'],
                            velocity = gt_box['velocity'],
                            num_pts = gt_box['num_pts'],
                            detection_name = gt_box['detection_name'],
                            detection_score = -1.0,
                            attribute_name=gt_box['attribute_name']
                            )
                        )
            all_gt.add_boxes(sample_token,sample_boxes)
